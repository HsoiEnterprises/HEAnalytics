...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func application(application: UIApplication, willFinishLaunchingWithOptions launchOptions: [NSObject : Any]?) -> Bool {
        SampleAnalytics.sharedInstance.start()
        return true
    }
...
extension AppDelegate : UITabBarControllerDelegate {
    ...
    func tabBarController(_ tabBarController: UITabBarController, didSelect viewController: UIViewController) {
        if tabBarController.selectedIndex == 0 {
            SampleAnalytics.sharedInstance.selectedTab1()
        }
        else if tabBarController.selectedIndex == 1 {
            SampleAnalytics.sharedInstance.selectedTab2()
        }
    }
}
